...
(define (compile-body exprs)
  ...
  (let ([compiled-exprs (map compile exprs)])
    (lambda ([env : ENV])
      (: new-eval : (Listof (ENV -> VAL)) -> VAL)
      (define (new-eval c-exprs)
        (let ([1st ((first c-exprs) env)]
              [rest-exprs (rest c-exprs)])
          (if (null? rest-exprs)
              1st
              (new-eval rest-exprs))))
  ...
  (cases expr
    ...
    [(Set name new)
     (let ([compiled-new (compile new)])
       (lambda ([env : ENV]) (set-box! (lookup name env) (compiled-new env))
         the-bogus-value))]
    [(Bind names exprs bound-body)
     ;(eval-body bound-body (extend names (map eval* exprs) env))
     (let ([compiled-exprs (map compile exprs)]
           [compiled-bound-body (compile-body bound-body)])
       (lambda ([env : ENV])
         (compiled-bound-body (extend names
    ...
    [(BindRec names exprs bound-body)
     (let ([compiled-bound-body (compile-body bound-body)]
       ...
       (lambda ([env : ENV])
         (compiled-bound-body (extend-rec names compiled-exprs env))))]
    ...
    [(Call fun-expr arg-exprs)
     (let ([compiled-fun-expr (compile fun-expr)]
           ...
           [compiled-get-boxes (compile-get-boxes arg-exprs)])
       (lambda ([env : ENV])
