...
(define (parse-sexpr sexpr)
  (match sexpr
    ...
    [(or (cons 'bind more) (cons 'bind* more))
     (match sexpr
       [(list 'bind (list (list (symbol: name) (sexpr: expr))
                          (list (symbol: names) (sexpr: exprs)) ...) body)
        (Bind (cons name names)
              (cons (parse-sexpr expr) (map parse-sexpr exprs))
              (parse-sexpr body))]
       [(list 'bind* (list (list (symbol: name) (sexpr: expr))
                           (list (symbol: names) (sexpr: exprs)) ...) body)
        (Bind* (cons name names)
...
(define (preprocess expr de-env)
  ...
  (cases expr
    ...
    [(Bind bound-ids bound-exprs bound-body)
     (sub (Call (Fun bound-ids bound-body) bound-exprs))]
    [(Bind* bound-ids bound-exprs bound-body)
     (if (= 1 (length bound-ids))
         (sub (Call (Fun bound-ids bound-body) bound-exprs))
         (sub (With (first bound-ids)
                    (first bound-exprs)
                    (Bind* (rest bound-ids) (rest bound-exprs) bound-body))))]
