...
(define (parse-sexpr sexpr)
  (match sexpr
    ...
    [(cons 'if0 more)
     (match sexpr
       [(list 'if0 val true-option false-option)
        (If0 (parse-sexpr val)
             (parse-sexpr true-option)
             (parse-sexpr false-option))]
       [else (error 'parse-sexpr "bad `if0' syntax in ~s" sexpr)])]
    [(cons 'with more)
     (match sexpr
       [(list 'with (list (symbol: name) named) body)
        (With name (parse-sexpr named) (parse-sexpr body))]
       [else (error 'parse-sexpr "bad `with' syntax in ~s" sexpr)])]
    ;; add rec parser to parse-sexpr
    [(cons 'rec more)
     (match sexpr
       [(list 'rec (list (symbol: name) function) body)
        (let ([f (parse-sexpr function)])
          (cases f
            [(Fun id f-body) (WRec name f (parse-sexpr body))]
...
(define (preprocess expr de-env)
  ...
  (cases expr
    ...
    [(If0 v t f) (CIf0 (sub v) (sub t) (sub f))]
